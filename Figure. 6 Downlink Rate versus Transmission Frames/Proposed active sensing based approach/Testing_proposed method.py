...
N_w_using = 30 #means each frame contains 30 blocks
...
# 'here, we want to couple the ht and hr in the same block!' # computing all the cascaded channel
...
###########################################################################################################################
'Compute the rand and optimal rate'
shape_h_compute_OpRand = (batch_size_test_1block, num_ris_elements)
h_compute_OpRand = np.zeros(shape_h_compute_OpRand, dtype=complex)

optimal_rate_perblock = []
rand_rate_perblock = []
for block in range(total_blocks):
    h_compute_OpRand[:, :] = hc_total[block * batch_size_test_1block: (block + 1) * batch_size_test_1block, :]

    'this is the ideal (maximum) average rate counting for this specific training frame, which containing all the channel stages'
    # since each frame will has its own rate
    optimal_v_val = np.conj(h_compute_OpRand / np.abs(h_compute_OpRand))
    y_tmp_val = np.sum(np.multiply(optimal_v_val, h_compute_OpRand), axis=1)
    optimal_rate_perblock.append(np.mean(np.log2(1 + (np.abs(y_tmp_val) ** 2) / noise_power_linear_down)))

    'if we use the random downlink beamforming vector to check the rate ( w/ perfect CSI)'
    rand_v_val = np.random.uniform(low=0, high=2*np.pi, size=(batch_size_test_1block, num_ris_elements))
    rand_v_val = np.exp(1j*rand_v_val)
    y_testing = np.sum(np.multiply(rand_v_val, h_compute_OpRand), axis=1)
    rand_rate_perblock.append(np.mean(np.log2(1 + (np.abs(y_testing) ** 2) / noise_power_linear_down)))
...
###########################################################################################################################
# save the data
optimal_rate_perframe = np.array(optimal_rate_perblock).reshape([total_blocks,1])
rand_rate_perframe = np.array(rand_rate_perblock).reshape([total_blocks,1])
trained_rate_perframe_LSTM_DNNdesign_v = np.array(trained_rate_perblock).reshape([total_blocks, 1])

print('optimal_rate_perframe', optimal_rate_perframe)
print('\n rand_rate_perframe',rand_rate_perframe)
print('\n trained_rate_perframe_LSTM_DNNdesign_v',trained_rate_perframe_LSTM_DNNdesign_v)


# save the recorded data
file_path_optimal_rate_perframe = 'optimal_rate_perframe.mat'
file_path_rand_rate_perframe = 'rand_rate_perframe.mat'
file_path_trained_rate_perframe_LSTM_DNNdesign_v = 'trained_rate_perframe_LSTM_DNNdesign_v.mat'

sio.savemat(file_path_optimal_rate_perframe, {'optimal_rate_perframe': optimal_rate_perframe})
sio.savemat(file_path_rand_rate_perframe, {'rand_rate_perframe': rand_rate_perframe})
sio.savemat(file_path_trained_rate_perframe_LSTM_DNNdesign_v, {'trained_rate_perframe_LSTM_DNNdesign_v': trained_rate_perframe_LSTM_DNNdesign_v})
